...
class Batchifier:
    ...
    def load_batch(self, randomize_batches, overlapping_batches, is_test):
        ...
        for batch_idx in range(num_batches):
            ...
            # X.shape => (bsz X bptt X num_features X num_assets), y.shape => (bsz X bptt X num_assets + 1)
            X = []
            y = []
            ...
            assert X.shape[1] == self.bptt and X.shape[2] == 3 and X.shape[3] == len(
                self.asset_list), "X shape: {}".format(X.shape)

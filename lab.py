...
if __name__=='__main__':
    dp = DataPreprocess()
    for train_iter, test_iter in dp.load_train_test(asset_name=["BTC_XEM", "BTC_LTC", "BTC_BTCD"],feature_type='close'):
        ts_data = 1000 * train_iter.values
        model = ARIMAModel(1, 1, 0, -1, 100)
        ytrue, ypred = model.evaluate(ts_data[:1000], 100)

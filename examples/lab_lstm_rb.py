...
from data_load.prioritized_replay_buffer import Priority_RB
...
if __name__ == '__main__':
    ...
    buffer = Priority_RB(capacity=replay)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1,NUM_EPOCHS + 1):
            ...
            for bTrainX, bTrainY in batch_gen.load_train():
                if buffer.tree.total() < buffer.capacity:
                    buffer.add(error=0, sample=bTrainY)
                    continue
                else:
                    ...
                    losses.append(loss)
                    if len(losses) % LOG_MINI_BATCH == 0:
                        print("Loss after Mini Batch {} Epoch {} = {}".format(len(losses) / LOG_MINI_BATCH, epoch,
                                                                              batch_losses / LOG_MINI_BATCH))
                        batch_losses = 0.0
                    else:
                        batch_losses += loss

...
BSZ=32
RB_FACTOR = 6 # REPLAY BUFFER FACTOR
BPTT=10
...
if __name__ == '__main__':
    ...
    model = LSTMModel(num_hid=NUM_HID, bptt=BPTT, num_assets=len(asset_list), lr=LR, scope_prefix="model", clip_norm=5.0)
    testModel = LSTMModel(num_hid=NUM_HID, bptt=BPTT, num_assets=len(asset_list), lr=LR, scope_prefix="testModel", clip_norm=5.0)
    # if MODEL == "CNN":
        ...
        for epoch in range(1,NUM_EPOCHS + 1):
            ...
            for idx, (bEvalX, bEvalY) in enumerate(batch_gen.load_test()):
                if buffer.size < buffer.max_size:
                    ...
                    buffer.add(state=bEvalX, action=bEvalY)
                    pred_allocations = sess.run(testModel.predict_portfolio_allocation,
                                                feed_dict={
                                                    testModel.data: np.expand_dims(bEvalX, axis=0)
                                                })

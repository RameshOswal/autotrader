...
class CNNModel:
    ...
    def build_model(self):
        ...
        self.conv_layers = [
            tf.layers.Conv2D(filters=self.n_channels[0], strides=(1, self.strides[0]),
                                          kernel_size=(1, self.kernel_sizes[0]), padding="SAME"),
            tf.layers.Conv2D(filters=self.n_channels[1], strides=(1, self.strides[1]),
    ...
    def loss(self):
        with tf.variable_scope(self._scope_prefix+"loss_op", reuse=tf.AUTO_REUSE):
            ...
            loss = tf.reduce_mean(log_probs, axis = 0)
            return loss
